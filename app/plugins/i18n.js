...
function detectLocaleFromAcceptedLanguages(
  acceptedLanguages,
  localesSupported
) {
  ...
  if (matchedLanguageFound) {
    const matchedLanguageCode = matchedLanguageFound.code;
    if (
      matchedLanguageFound.region &&
      localesSupported.includes(
        matchedLanguageFound.code + '-' + matchedLanguageFound.region
      )
    ) {
  ...
  register: function (server, options) {
    ...
    server.ext('onPreAuth', (request, h) => {
      ...
      if (request.params && request.params.languageCode) {
        ...
        const matchedLanguageCode = detectLocaleFromAcceptedLanguages(
          request.headers[pluginOptions.languageHeaderField],
          pluginOptions.locales
        );
